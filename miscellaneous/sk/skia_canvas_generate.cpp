  ...
  void addFlag(bool flag_val, const char flag_name[]) 
{
    ...
    if (!oss_.str().empty()) oss_ << separator_;
...
base::Value AsValue(const SkPaint& paint) {
  ...
  if (paint.getColor() != default_paint.getColor()) val.Set("Color", AsValue(paint.getColor()));
  ...
  if (paint.getColorFilter()) val.Set("ColorFilter", AsValue(*paint.getColorFilter()));
...
base::Value AsValue(const SkPath& path) {
  ...
  static_assert(std::size(gVerbStrings) == static_cast<size_t>(SkPath::kDone_Verb + 1),
...
void BenchmarkingCanvas::onClipRect(const SkRect& rect, SkClipOp region_op,
...
void BenchmarkingCanvas::onClipRRect(const SkRRect& rrect, SkClipOp region_op,
                                     SkCanvas::ClipEdgeStyle style) {
...
void BenchmarkingCanvas::onClipPath(const SkPath& path, SkClipOp region_op,
                                    SkCanvas::ClipEdgeStyle style) {
